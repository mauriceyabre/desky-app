                ...
                <table class="table table-responsive table-row-bordered m-0" v-if="!!filteredProjects.length">
                    <tbody>
                        <tr class="cursor-pointer" v-for="project in filteredProjects" :key="project.id" v-if="filteredProjects.length" @click.prevent="selectProject(project)">
                            ...
                            <td class="text-end">
                                <span class="badge fs-7 px-4 py-3" :class="project.has_worked_on ? 'badge-light-primary' : 'badge-light'">{{ moment(project.started_at).format('MMM YYYY').capitalize() }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="text-center py-10" v-else :class="{'opacity-50': isLoading}">
...
<script setup lang="ts">
    ...
    const projects = ref<{id: number, name: string, started_at: string, has_worked_on: boolean, customer: { name: string }}[]>([])
    const element = ref()
    const selectEl = ref()
    const search = ref('')
    ...
    async function fetchProjects(search: string = '') {
        ...
        return await axios.get(route('projects.fetch'), { params: { search, user_id: userId.value }}).then((res) => {
            return Object.values(res.data.projects).map((item: any) => {
                return {
                    id: item.id as number,
                    name: item.name as string,
                    started_at: item.started_at as string,
                    has_worked_on: item.has_worked_on as boolean,
                    customer: { name: item.customer.name.toLowerCase().capitalize() as string }
                }
            })
